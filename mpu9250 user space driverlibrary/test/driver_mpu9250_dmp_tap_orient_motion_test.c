/**
 *
 * @file      driver_mpu9250_dmp_tap_orient_motion_test.c
 * @brief     driver mpu9250 dmp tap orient motion test source file
 * @version   1.0.0
 * @author    Nguyen Nhan
 * @date      2025-08-30
 *
...
uint8_t mpu9250_dmp_tap_orient_motion_test(mpu9250_interface_t interface, mpu9250_address_t addr)
{
    ...
    DRIVER_MPU9250_LINK_RECEIVE_CALLBACK(&gs_handle, a_receive_callback);
    ...
    mpu9250_interface_debug_print("mpu9250: start dmp tap orient motion test.\n");
    ...
    /* set 200mg */
    res = mpu9250_motion_threshold_convert_to_register(&gs_handle, 200.0f, &reg);
    if (res != 0)
    {
    ...
    res = mpu9250_dmp_set_tap_callback(&gs_handle, a_dmp_tap_callback);
    if (res != 0)
    {
    ...
    res = mpu9250_dmp_set_orient_callback(&gs_handle, a_dmp_orient_callback);
    if (res != 0)
    {
    ...
    mpu9250_interface_debug_print("mpu9250: finish dmp tap orient motion test.\n");
    (void)mpu9250_deinit(&gs_handle);
